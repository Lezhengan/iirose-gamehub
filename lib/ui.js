...
(function () {
  ...
  /* ---------- 侧边栏按钮 ---------- */
  var _sidebarRetries = 0;
  var _SIDEBAR_MAX_RETRIES = 15; // 最多等 30 秒
  ...
  function insertSidebarButton() {
    ...
    for (var i = 0; i < groups.length; i++) {
      ...
      button.addEventListener('click', function () { GhUI.toggle(); });
      box.insertBefore(button, box.children[1] || null);
      return true;
    }
    // 如果没找到"工具"分组，延迟重试（上限 15 次）
    _sidebarRetries++;
    if (_sidebarRetries < _SIDEBAR_MAX_RETRIES) setTimeout(insertSidebarButton, 2000);
    return false;
  }
  ...
  function createFAB() {
    ...
    fab.textContent = '\uD83C\uDFAE';
    fab.title = '\u6E38\u620F\u5927\u5385';
    // 内联样式兜底: CSS 加载完成前按钮可见
    fab.style.cssText = 'position:fixed;bottom:28px;right:28px;z-index:2147483646;width:56px;height:56px;border-radius:50%;border:none;cursor:pointer;background:linear-gradient(135deg,#f6a,#f8a);box-shadow:0 4px 16px rgba(255,102,170,0.4);font-size:28px;display:flex;align-items:center;justify-content:center;';
    fab.addEventListener('click', function () { GhUI.toggle(); });
    dom.fab = fab;
  ...
  function createPanel() {
    ...
    closeBtn.addEventListener('click', function () { GhUI.hide(); });
  ...
  function createModal() {
    ...
    overlay.addEventListener('click', function (e) { if (e.target === overlay) GhUI.closeModal(); });
    ...
    closeBtn.addEventListener('click', function () { GhUI.closeModal(); });
  ...
  var GhUI = {
    ...
    toggle: function () { isVisible ? GhUI.hide() : GhUI.show(); },
